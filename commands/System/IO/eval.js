...
module.exports = class extends Command {
	...
	timedEval(message, code, flagTime) {
		if (flagTime === Infinity || flagTime === 0) return this.evaluate(message, code);
		return Promise.race([
	...
	async evaluate(message, code) {
		...
		try {
			...
			const np = message.guild.player.nowPlaying;
			result = eval(code);
		...
		if (typeof result !== 'string') {
			result = result instanceof Error ? result.stack : message.flagArgs.json ? JSON.stringify(result, null, 4) : inspect(result, {
